...
public abstract class AbstractHandler implements RequestHandler<GatewayRequest, GatewayResponse> {
    ...
    protected GatewayResponse gatewayResponse(int statusCode, Object body) {
        return GatewayResponse.builder().statusCode(statusCode).body(GsonHelper.gson().toJson(body)).headers(GatewayResponse.HEADERS_JSON).build();
    }
    ...
    protected GatewayResponse internalServerError() {
        return this.gatewayResponse(500, this.errorResponse(String.valueOf(500), "Internal Server Error, Please contact administrator."));
    }
