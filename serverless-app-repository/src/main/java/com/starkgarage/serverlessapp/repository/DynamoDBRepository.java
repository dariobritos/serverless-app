...
public class DynamoDBRepository<V> implements IRepository<V> {
    ...
    public DynamoDBRepository(final AmazonDynamoDB dynamoDBClient
            , final Class<V> type, final String tableName) {
        this.dynamoDBMapper = new DynamoDBMapper(dynamoDBClient, new DynamoDBMapperConfig.Builder()
    ...
    public V findById(String id) {
        PaginatedQueryList<V> results = dynamoDBMapper.query(type,
                new DynamoDBQueryExpression<V>().withKeyConditionExpression("id = :id")
                        .withExpressionAttributeValues(ImmutableMap.of(":id", new AttributeValue().withS(id))));
        if (!results.isEmpty()) {
